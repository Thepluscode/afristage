...
// Register returns the same { accessToken, refreshToken } as login, so a new user
// is signed in immediately (straight from watching to a wallet).
export async function POST(req: NextRequest) {
  ...
  if (!res.ok) {
    ...
    return NextResponse.json({ message: err?.message || 'Registration failed' }, { status: res.status === 409 ? 409 : 400 });
  }
