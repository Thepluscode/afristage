...
describe('LiveRoomsController', () => {
  it('delegates every endpoint (upcoming default + explicit)', () => {
    const s = { create: jest.fn(), start: jest.fn(), end: jest.fn(), list: jest.fn(), upcoming: jest.fn(), get: jest.fn(), joinToken: jest.fn(), guestToken: jest.fn(), setReminder: jest.fn(), cancelReminder: jest.fn() };
    ...
    c.join(u, 'r1'); c.guestToken('r1'); c.setReminder(u, 'r1'); c.cancelReminder(u, 'r1');
    ...
    expect(s.guestToken).toHaveBeenCalledWith('r1'); // public, no user arg
  });
});
