...
| NestJS 12 and its ESM-compatible test/runtime stack replace the vulnerable NestJS 10 baseline. | IMPLEMENTED | Nest packages are aligned on v12, TypeScript 6 uses NodeNext resolution, Node is pinned to `24.21.0`, Jest 30 runs the ESM-only framework packages, and Express/UUID/Vitest were upgraded with the same dependency sweep. Local evidence: API **95/95 suites, 1,112/1,112 tests**, web **15/15 files, 76/76 tests**, admin **42/42 files, 465/465 tests**, all three production builds pass, and `npm audit` reports **0 vulnerabilities**. Deployment/runtime evidence is still required before VERIFIED. |
