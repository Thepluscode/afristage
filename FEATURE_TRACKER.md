...
| Every runtime and service version is pinned; Node 20 (EOL April 2026) retired (#275). | VERIFIED | `node -v` over `railway ssh` on api, web, admin-web: `v24.21.0`. Deployed api image holds no `.env`. CI migrations run on Postgres 18.6, matching the deployed database. Dependabot proposes grouped updates monthly. |
| NestJS 12 and its ESM-compatible test/runtime stack replace the vulnerable NestJS 10 baseline. | VERIFIED | Production API deployment `bf555e8c-8d7c-45e9-9807-63b21df58ca8` serves commit `f61e42ecaec2c853aedc9767529a0241d6c79c37`; `railway ssh` reports Node `v24.21.0`, Nest `12.1.0`, and Express `5.2.1`. Main workflow `36008745022` passed build/test, deployed API/admin/web, and verified the API and web commit endpoints; independent API and web health probes returned the same commit. Regression evidence: API **95/95 suites, 1,112/1,112 tests**, web **15/15 files, 76/76 tests**, admin **42/42 files, 465/465 tests**, all three production builds pass, and full `npm audit` reports **0 vulnerabilities**. |
| Staging Redis is reachable only on the private network and runs the patched image. | VERIFIED | Railway reports **no TCP proxies** and no public URL for Redis. Deployment `508f847b-0e22-4d1c-bf1d-e61978e54470` is live on `redis:8.2.10` (`redis_version:8.2.10`); authenticated `PING` returned `PONG`, the persisted database remained present, and the dependent API health endpoint stayed `200`. |
| Railway deployment configuration is migrated from deprecated Config as Code to project-wide IaC. | VERIFIED | `.railway/railway.ts` was imported from the live production environment with secret values represented only by `preserve()`. A pinned plan moved the four application services' existing Dockerfile, health-check, migration, and restart settings from deployment-time `railway.toml` into service configuration with **0 adds / 0 destroys**; the post-migration plan is a no-op. `railway.toml` is removed before Railway stops reading it on 2026-12-01. |
...
| Native viewer/creator workflow regression and design-capture suite. | IMPLEMENTED | `flutter analyze` reports no issues; full `flutter test --reporter compact` exited 0 with **116 passed / 1 capture-only skipped**; deterministic design captures passed for home, live room, Go Live, creator dashboard, and wallet. Physical camera/mic publishing is staging-verified; VoiceOver validation remains outstanding. |
...
| Creator media failure recovery is actionable. | VERIFIED (staging) | The deployed Flutter client completed login → Go Live → camera/mic publish with an authenticated creator in Chrome on a physical Mac camera. The local preview showed live video; LiveKit `RoomServiceClient` reported the participant connected with two unmuted tracks (`source: CAMERA` video and `source: MICROPHONE` audio). The signed-out deployed `/watch` viewer displayed that camera feed, counted both participants, and its “Tap for sound” control successfully enabled audio. Cleanup ended the API room and left **0 live API rooms / 0 LiveKit participants**. Existing recovery coverage remains **120/120** focused tests with `flutter analyze` clean. |
...
| **LiveKit Cloud wired + verified on staging**: API issues accepted tokens; the mobile app connects to LiveKit Cloud through the full product flow (login → Go Live → publish button) — participant visible server-side; real camera and microphone media publish into app-created rooms | VERIFIED | Physical Mac camera/mic drill on 2026-09-24: deployed Flutter host preview rendered live video; `RoomServiceClient` reported the participant plus unmuted CAMERA and MICROPHONE tracks; the signed-out deployed `/watch` viewer rendered the same live feed and enabled audio. Cleanup left 0 live API rooms and 0 LiveKit participants. | #172 |
...
check, `RESEND_API_KEY` to light up email delivery. Remaining wave-1 check:
imagery provenance for the `/site` marketing photos before any marketing push
(#171). Physical camera/mic publishing and signed-out playback are staging-verified.

| **Public viewer CORS/empty-state hardening** — deployed `/watch` remained indefinitely on “Finding a live stage…” because browser discovery called the API cross-origin while staging omitted the web origin from CORS. Added a narrow same-origin `/api/public-live/*` proxy (allow-listed room GETs, top-gifters, and guest-token POST), switched browser wiring to it, and surfaced rejected discovery/token requests as a visible failure instead of an infinite loader. | VERIFIED (staging) | Railway deployment `47c951cf-2729-4e1a-a201-91b5b3406159` succeeded. Empty-state and proxy restrictions remain covered by **41/41** web tests. Real playback was proven on 2026-09-24: a signed-out deployed `/watch` session discovered the staging room, displayed the physical camera feed, showed two participants, and removed “Tap for sound” after audio enablement; LiveKit independently reported unmuted camera and microphone tracks. |
| **Public viewer live-state overlay** — real playback exposed a race where a video track subscribed during `Room.connect()`, cleared the loading state, and was then covered again by “Waiting for the stage…” when the connection promise resolved. The viewer now records the video subscription independently and only shows the waiting state when no video has arrived. | TESTED | Regression reproduces a video subscription during `connect()` and proves neither loading overlay is restored. Web suite **77/77**, 100% statements/branches/functions/lines, and optimized production build pass. Deployed playback re-check pending. |
...
| **Flutter authentication form clarity and registration spacing** — login and registration now keep field labels visible, state what information is expected and how it is used, expose password/profile requirements, provide autofill and keyboard actions, and size each registration step to its content instead of reserving a fixed 360px blank area. The registration app bar no longer overlays the form header. | VERIFIED (staging) | Flutter suite **411 passed / 1 capture-only skipped**, `flutter analyze` clean, release web build passed. A 390×844 widget regression enforces a field-to-Continue gap of at most **40px**. Railway deployment `e5735920-0654-4ed5-977a-f168e4302555` succeeded; live browser evidence shows the login guidance and the Create Account screen with email privacy copy, the 8-character password requirement, Continue immediately after the fields, and legal links visible in the same mobile viewport. |
| **Mobile BIGO-fidelity hierarchy pass** — Viewer Home now leads with the featured performance and follows live rooms → categories → creators before recommendations and wallet utilities; creator navigation selected states use broadcast purple; Wallet uses a white primary balance and one restrained purple menu accent system. No backend, ledger, or payout behavior changed. | VERIFIED (staging) | Flutter suite **412 passed / 1 capture-only skipped**, `flutter analyze` clean, release web build passed, focused UI suite **25/25**, and the capture harness regenerated five 390×844 states. The same-input source/current board is `qa-mobile-reference-current.png`; `design-qa.md` records no remaining P0/P1/P2 mismatch. Railway deployment `558589f4-469f-411e-bc84-46eb8f3c92ef` succeeded; `/api/health` returned **200** with exact commit `0219310bed8d24eb6c4b3d3f32d641752a2582f8`; a live browser render showed the performance-first Home hierarchy and role-aware purple creator navigation with zero console warnings/errors. Physical-device validation remains part of the broader native-release gate, not this hosted-web claim. |
